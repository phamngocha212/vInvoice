<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Home.Master" Inherits="System.Web.Mvc.ViewPage<PSVTransactionIndexModels>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="box box-primary">
        ...
        <div class="box-body">
            <div class="row">
                <div class="col-xs-11">
                    <form id="Searchform" method="post" class="form-inline" action="/PsvTransaction/Index?TypeTran=<%=ViewData["TypeTran"]%>">
                        <%=Html.Hidden("TypeTran", ViewData["TypeTran"]) %>
                ...
                <div class="col-sm-1">
                    <a href="/PsvTransaction/Upload?TypeTran=<%=ViewData["TypeTran"] %>" class="btn btn-info btn-sm element-right"><i class="fa fa-plus"></i>&nbsp;&nbsp;Tạo mới</a>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td style="text-align: center">
                                    <%if (cus.Status != TranSactionStatus.NewUpload && cus.Status != TranSactionStatus.Processing && cus.Status != TranSactionStatus.NotComplete && cus.Status != TranSactionStatus.Failed)
                                      {%>
                                    <a class="dtrans" href="/PsvTransaction/Completed/<%=cus.id%>" title="Download">
                                ...
                                <td style="text-align: center">
                                    <%if (cus.Status == TranSactionStatus.NotComplete || cus.Status == TranSactionStatus.Failed)
                                      {%>
                                    <a class="dftrans" href="/PsvTransaction/Download/<%=cus.id%>" title="Download">
                                        <i class="fa fa-download"></i></a>
                                    <%}
                                      else
                                      { %>
                                ...
                                <td style="text-align: center"><a href="/PsvTransaction/Details?id=<%=cus.id%>&TypeTran=<%=ViewData["TypeTran"] %>" title="Edit">
    ...
    <script language="javascript" type="text/javascript">
        function OnDelete(id) {
            ...
            alertify.confirm("Xoá giao dịch?", function (e) {
                if (e) {
                    document.location = "/PsvTransaction/delete?id=" + id + '&TypeTran=' + typetran;
                }
                else return;
            });
        }
        ...
        function OnCompare(id) {
            if (!confirm("Chấp nhận đối soát?"))
                return;
            document.location = "/PsvTransaction/compare?id=" + id;
        }
